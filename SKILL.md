...
一体化"比价 + verdict 推荐"的薄壳层 skill。**不重写爬虫**，从 `shopmind-price-compare` import 数据层 primitives（HEADERS / API / `calc_savings` / `PLATFORM_MAP`），跳过其文本拼接阶段直接拿原始 `items[]`，再按价格分布跑 verdict 规则。

## 触发词

比价、值不值得买、值不值、历史价、套路价、哪个平台最便宜、X 平台最便宜、这个多少钱合适、当前价合理吗、慢慢买、price compare、是不是先涨后降
